...
export const data = new SlashCommandBuilder()
	...
	.addSubcommand((subcommand) =>
		subcommand
			.setName('add')
			.setDescription(
				'Add your cake day so i can let people know its your day',
			)
			.addNumberOption((num) =>
				num
					.setName('day')
...
export const execute = async (interaction: ChatInputCommandInteraction) => {
	...
	if (userDetails === null) {
		await interaction.editReply(
			`I'm really sorry, but i don't know you yet. If you start chatting with others for a bit, then you can add your birthday.`,
		);
		return;
	}
	...
	if (interaction.options.getSubcommand() === 'add') {
		...
		try {
			...
			await interaction.editReply(
				`I've saved ${birthDate}/${birthMonth} as your birthday, and will let everyone know when your day arrives`,
			);
		} catch (error) {
		...
		if (entry === null) {
			await interaction.editReply(
				'I have removed your cake day from my reminder list',
			);
			return;
		}
